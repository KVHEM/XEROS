...
for (i in 1:nrow(list_files_d)) {
  ...
  download.file(file_url_d[i], paste0(path_d, name) )
}
...
for (i in 1:nrow(list_files_m)) {
  ...
  download.file(file_url_m[i], paste0(path_m, name) )
}
...
for (i in 1:length(list_names_d)) {
  ...
  daily_precip [ , lon := meta[,3]]
  daily_precip [ , lat := meta[,4]]
  daily_precip [ , id := i]
...
all_daily_precip <- as.data.frame(all_daily_precip)  # change format of lon lat and precip to numeic
...
setnames( metadata_d_precip, old = c('V10', 'V11', 'V31', 'V41'), new= c('station_name', 'station_country', 'lon', 'lat')) # change format in metadata file
metadata_d_precip <- as.data.frame(metadata_d_precip)
metadata_d_precip$station_country[ metadata_d_precip$station_country == 'United'] <- 'UK'
...
for (i in 1:length(list_names_m)) {
  ...
  setnames(monthly_precip, old = c('V1', 'V2', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8', 'V9', 'V10', 'V11', 'V12', 'V13'),
  ...
  monthly_precip [ , lon := meta[,3]]
  monthly_precip [ , lat := meta[,4]]
  monthly_precip [ , id := i]
